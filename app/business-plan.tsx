...
// Adjusted to be monthly values instead of annually
const BASE_VALUES = {
  initialInvestment: 100000,  // Initial investment for 1 hectare
  monthlyMaintenanceCost: 1667,  // Monthly maintenance cost per hectare (20000/12)
  monthlyCropRevenue: 3750,  // Monthly revenue per hectare (45000/12)
  laborCosts: 1250,  // Monthly labor costs per hectare (15000/12)
  waterCosts: 667,  // Monthly water costs per hectare (8000/12)
  fertilizersAndPesticidesCosts: 583,  // Monthly costs for fertilizers and pesticides (7000/12)
  equipmentDepreciation: 417,  // Monthly equipment depreciation (5000/12)
};

// Default profile values
const DEFAULT_PROFILE = {
  region: 'fes-meknes', // Default region from profile
  soilType: 'loamy', // Default soil type from profile
  waterType: 'irrigation', // Default water type from profile
};

// The current date
const CURRENT_DATE = '2025-05-05';

export default function BusinessPlanScreen() {
  const router = useRouter();
  const [region, setRegion] = useState(DEFAULT_PROFILE.region);
  const [waterType, setWaterType] = useState(DEFAULT_PROFILE.waterType);
  const [soilType, setSoilType] = useState(DEFAULT_PROFILE.soilType);
  ...
  const navigateBack = () => {
    try {
      ...
      // Try alternative navigation if the first method fails
      router.push('/(tabs)');
    }
  };
    ...
    const loadUserProfile = async () => {
      try {
        ...
        if (profile) {
          ...
          if (parsedProfile.region) setRegion(parsedProfile.region);
          if (parsedProfile.soilType) setSoilType(parsedProfile.soilType);
          if (parsedProfile.waterType) setWaterType(parsedProfile.waterType);
        }
      } catch (error) {
  ...
  const calculateProjections = () => {
    ...
    // Calculate base scenario - MONTHLY values
    const baseInitialInvestment = BASE_VALUES.initialInvestment * regionFactor.costFactor * areaInHectares;
    const baseMonthlyMaintenance = BASE_VALUES.monthlyMaintenanceCost * regionFactor.costFactor * waterFactor.costFactor * areaInHectares;
    ...
    const baseTotalMonthlyCosts = baseMonthlyMaintenance + baseWaterCosts + baseFertilizerCosts + baseLaborCosts + baseDepreciation;
    const baseMonthlyRevenue = BASE_VALUES.monthlyCropRevenue * crop.revenueFactor * regionFactor.baseYield * 
    ...
    const baseMonthlyProfit = baseMonthlyRevenue - baseTotalMonthlyCosts;
    const baseROI = ((baseMonthlyProfit * 6) / baseInitialInvestment) * 100; // 6-month ROI
    const baseBreakEvenMonths = baseMonthlyProfit > 0 ? baseInitialInvestment / baseMonthlyProfit : 'N/A';
    ...
    const upMonthlyMaintenance = baseMonthlyMaintenance;
    ...
    const upTotalMonthlyCosts = upMonthlyMaintenance + upWaterCosts + upFertilizerCosts + upLaborCosts + upDepreciation;
    const upMonthlyRevenue = baseMonthlyRevenue * upMult;
    const upMonthlyProfit = upMonthlyRevenue - upTotalMonthlyCosts;
    const upROI = ((upMonthlyProfit * 6) / upInitialInvestment) * 100; // 6-month ROI
    const upBreakEvenMonths = upMonthlyProfit > 0 ? upInitialInvestment / upMonthlyProfit : 'N/A';
    ...
    const downMonthlyMaintenance = baseMonthlyMaintenance * 1.1; // Higher maintenance costs
    ...
    const downTotalMonthlyCosts = downMonthlyMaintenance + downWaterCosts + downFertilizerCosts + downLaborCosts + downDepreciation;
    const downMonthlyRevenue = baseMonthlyRevenue * downMult;
    const downMonthlyProfit = downMonthlyRevenue - downTotalMonthlyCosts;
    const downROI = ((downMonthlyProfit * 6) / downInitialInvestment) * 100; // 6-month ROI
    const downBreakEvenMonths = downMonthlyProfit > 0 ? downInitialInvestment / downMonthlyProfit : 'N/A';

    // Return projections for all three scenarios with 6 month cash flow
    return {
      base: {
        initialInvestment: Math.round(baseInitialInvestment),
        monthlyCosts: Math.round(baseTotalMonthlyCosts),
        monthlyRevenue: Math.round(baseMonthlyRevenue),
        monthlyProfit: Math.round(baseMonthlyProfit),
        roi: typeof baseROI === 'number' ? baseROI.toFixed(1) : 'N/A',
        breakEvenMonths: typeof baseBreakEvenMonths === 'number' ? baseBreakEvenMonths.toFixed(1) : 'N/A',
        cashFlow: [
          { month: 1, value: -baseInitialInvestment + baseMonthlyProfit },
          { month: 2, value: (-baseInitialInvestment + baseMonthlyProfit * 2) },
          { month: 3, value: (-baseInitialInvestment + baseMonthlyProfit * 3) },
          { month: 4, value: (-baseInitialInvestment + baseMonthlyProfit * 4) },
          { month: 5, value: (-baseInitialInvestment + baseMonthlyProfit * 5) },
          { month: 6, value: (-baseInitialInvestment + baseMonthlyProfit * 6) }
        ]
      },
      upside: {
        initialInvestment: Math.round(upInitialInvestment),
        monthlyCosts: Math.round(upTotalMonthlyCosts),
        monthlyRevenue: Math.round(upMonthlyRevenue),
        monthlyProfit: Math.round(upMonthlyProfit),
        roi: typeof upROI === 'number' ? upROI.toFixed(1) : 'N/A',
        breakEvenMonths: typeof upBreakEvenMonths === 'number' ? upBreakEvenMonths.toFixed(1) : 'N/A',
        cashFlow: [
          { month: 1, value: -upInitialInvestment + upMonthlyProfit },
          { month: 2, value: (-upInitialInvestment + upMonthlyProfit * 2) },
          { month: 3, value: (-upInitialInvestment + upMonthlyProfit * 3) },
          { month: 4, value: (-upInitialInvestment + upMonthlyProfit * 4) },
          { month: 5, value: (-upInitialInvestment + upMonthlyProfit * 5) },
          { month: 6, value: (-upInitialInvestment + upMonthlyProfit * 6) }
        ]
      },
      downside: {
        initialInvestment: Math.round(downInitialInvestment),
        monthlyCosts: Math.round(downTotalMonthlyCosts),
        monthlyRevenue: Math.round(downMonthlyRevenue),
        monthlyProfit: Math.round(downMonthlyProfit),
        roi: typeof downROI === 'number' ? downROI.toFixed(1) : 'N/A',
        breakEvenMonths: typeof downBreakEvenMonths === 'string' ? downBreakEvenMonths : downBreakEvenMonths.toFixed(1),
        cashFlow: [
          { month: 1, value: -downInitialInvestment + downMonthlyProfit },
          { month: 2, value: (-downInitialInvestment + downMonthlyProfit * 2) },
          { month: 3, value: (-downInitialInvestment + downMonthlyProfit * 3) },
          { month: 4, value: (-downInitialInvestment + downMonthlyProfit * 4) },
          { month: 5, value: (-downInitialInvestment + downMonthlyProfit * 5) },
          { month: 6, value: (-downInitialInvestment + downMonthlyProfit * 6) }
        ]
      }
    };
  };
  ...
  return (
    <SafeAreaView style={styles.container} edges={['top']}>
      <Stack.Screen
        options={{
          title: 'Business Plan',
          headerShown: false, // Hide the header to use our custom one
        }}
      />
      ...
      <View style={styles.customHeader}>
        ...
        <Text style={styles.headerTitleText}>Business Plan</Text>
      ...
      <ScrollView showsVerticalScrollIndicator={false} style={styles.scrollView}>
        <View style={styles.header}>
          ...
          <Text style={styles.headerTitle}>6-Month Agricultural Business Plan</Text>
        </View>
        ...
        <View style={styles.infoCard}>
          ...
          <Text style={styles.infoDescription}>
            Provide details about your farm to generate customized monthly business projections for a 6-month period.
          </Text>
        </View>
        ...
        {projections && currentScenario ? (
          <View style={styles.resultsSection}>
            ...
            <View style={styles.resultCard}>
              <Text style={styles.resultCardTitle}>Financial Overview (6-Month Plan)</Text>
              ...
              <View style={styles.resultRow}>
                <Text style={styles.resultLabel}>Monthly Costs:</Text>
                <Text style={styles.resultValue}>{currentScenario.monthlyCosts.toLocaleString()} MAD</Text>
              </View>
              
              <View style={styles.resultRow}>
                <Text style={styles.resultLabel}>Monthly Revenue:</Text>
                <Text style={styles.resultValue}>{currentScenario.monthlyRevenue.toLocaleString()} MAD</Text>
              </View>
              ...
              <View style={styles.resultRow}>
                ...
                <Text style={[
                  styles.resultValue, 
                  currentScenario.monthlyProfit > 0 ? styles.profitPositive : styles.profitNegative
                ]}>
                  {currentScenario.monthlyProfit.toLocaleString()} MAD
                </Text>
              </View>
              
              <View style={styles.resultRow}>
                <Text style={styles.resultLabel}>6-Month ROI:</Text>
                <Text style={[
                  styles.resultValue, 
              ...
              <View style={styles.resultRow}>
                <Text style={styles.resultLabel}>Break-even (months):</Text>
                <Text style={styles.resultValue}>
                  {currentScenario.breakEvenMonths}
                </Text>
              </View>
            </View>
            ...
            {chartData && (
              <View style={styles.chartCard}>
                <Text style={styles.chartTitle}>6-Month Cash Flow Projection (in thousands MAD)</Text>
            ...
            <View style={styles.scenarioDescriptionCard}>
              ...
              <Text style={styles.scenarioDescription}>
                {scenarioType === 'base' && (
                  `This base case scenario assumes standard market conditions, average crop yields, and typical operational costs for your selected region and farm characteristics. It represents the most likely outcome based on current data for a 6-month agricultural business plan.`
                )}
                {scenarioType === 'upside' && (
                  `This upside scenario represents an optimistic outlook with better-than-expected crop yields (20% above base case), favorable market conditions, and potential cost savings. This could occur with ideal weather and market conditions over the 6-month period.`
                )}
                {scenarioType === 'downside' && (
                  `This conservative scenario accounts for potential risks such as lower crop yields (20% below base case), market volatility, higher operational costs, or challenging weather conditions. It helps prepare for contingencies over the 6-month farming period.`
                )}
              </Text>
            </View>
            ...
            <View style={styles.recommendationsCard}>
              ...
              <Text style={styles.recommendation}>
                {currentScenario.monthlyProfit > 0 ?
                  `• Based on projections, this 6-month agricultural operation is profitable with a ${currentScenario.roi}% ROI.` :
                  '• This scenario shows challenges in profitability. Consider adjusting your crop selection or operational approach.'}
              </Text>
              
              <Text style={styles.recommendation}>
                {currentScenario.breakEvenMonths !== 'N/A' && parseFloat(currentScenario.breakEvenMonths) <= 6 ?
                  `• Your investment should break even within ${currentScenario.breakEvenMonths} months, which is within the 6-month plan period.` :
                  `• This plan may require a longer-term approach to achieve profitability beyond the initial 6-month period.`}
              </Text>
            </View>
          </View>
        ) : (
          <View style={styles.emptyResults}>
            <Text style={styles.emptyResultsText}>
              Enter all farm details to see your 6-month business plan projections
            </Text>
          </View>
        )}
        
        {/* Bottom padding to ensure content isn't blocked by fixed home button */}
...
const styles = StyleSheet.create({
  ...
  bottomPadding: {
    height: 80, // Space for the fixed home button
  },
  scrollView: {
    flex: 1,
  },
